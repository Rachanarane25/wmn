...
function initExperiments() {
  ...
  panels.innerHTML = experiments.map((exp, index) => `
    <article class="exp-panel ${index === 0 ? 'active' : ''}" id="experiment-${exp.number}">
      ...
      ${createCollapsible('Aim', 'aim', `<p>${exp.aim}</p>`, true)}
      ${createCollapsible('Objectives', 'objectives', listMarkup(exp.objectives), true)}
      ${createCollapsible('Theory', 'theory', `<p>${exp.theory}</p>`)}
      ${createCollapsible('Procedure', 'procedure', listMarkup(exp.procedure))}
      ${createCollapsible('Screenshots Section', 'screenshots', `
        <div class="screenshot-grid">
          <div class="screenshot-placeholder"><span class="icon">▧</span><span>Add topology screenshot</span></div>
          <div class="screenshot-placeholder"><span class="icon">▧</span><span>Add terminal output</span></div>
        </div>
      `)}
      ${createCollapsible('Results', 'results', `<p>${exp.results}</p>`, true)}
      ${createCollapsible('Conclusion', 'conclusion', `<p>${exp.conclusion}</p>`)}
    </article>
  `).join('');
